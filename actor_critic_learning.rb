...
class Model
  ...
  attr_accessor :state_values, :state_action_logprobs, :score_log
  ...
  def perform # rubocop:disable Metrics/MethodLength
    1000.times do
      ...
      temporal_difference_update(next_value:, rewards: reward_log, states: state_log)

      state_returns = calc_returns(rewards: reward_log)
      advantage = Torch.tensor(state_returns) - Torch.tensor(values_log)
...
model = Model.new(state_transitions:, rewards:)
model.perform
...
puts UnicodePlot.lineplot(
  Array(0..model.score_log.length.pred),
  model.score_log.map(&:to_f),
  ylim: [-0.1, 1.1],
  width: 70,
  height: 12
)
